    ...
    __call = function(self)
        ...
        self.save = function(column, where, string, data)
            ...
            while self.busy[string] and self.busycd[string] and self.busycd[string] < 100 do 
                if self.busycd[string] then self.busycd[string] = self.busycd[string] + 1 end                Wait(10) 
            end
        ...
        self.updateall = function(data, plate)
            ...
            while self.busy[plate] and self.busycd[plate] and self.busycd[plate] < 100 do 
                if self.busycd[plate] then self.busycd[plate] = self.busycd[plate] + 1 end                                Wait(1)
            end
...
Citizen.CreateThreadNow(function()
    ...
    local success, result = pcall(MySQL.scalar.await, 'SELECT `nodegrade` FROM renzu_tuner') -- check if nodegrade column exists
    if not success then
