...
lib.callback.register('renzu_tuners:vehiclestats', function(src, plate)
    local inventoryItems = GetInventoryItems(src, 'slots', plate) -- Fetch inventory items related to the vehicle
    local stats = { [plate] = vehiclestats[plate] or {}, inventory = inventoryItems } -- Include inventory items in the stats
...
CreateThread(function()
    ...
    default_handling = cache.defaulthandling or {}
...
if ent.default_handling then
    default_handling[plate] = ent.default_handling
    ...
    while true do
        Wait(20000)
        for k, v in pairs(default_handling) do
if not isPlateOwned(k) and not config.debug then
    default_handling[k] = nil
...
lib.callback.register('renzu_tuners:Tune', function(src, data)
    local inventoryItems = GetInventoryItems(src, 'slots', data.vehicle) -- Fetch inventory items for the vehicle
    ...
    -- Check if the player has the required items in their inventory for tuning
    if not inventoryItems or #inventoryItems == 0 then
        return false -- Not enough items to tune
    end
    ...
    if not config.purchasableUpgrade then
        ...
        if items then
            for k, v in pairs(items) do
                if metadata and isItemMetadata and v.metadata and v.metadata.upgrade == item or not isItemMetadata and not v.metadata or not metadata then
        ...
        if chance and math.random(1, 100) <= chance or not chance then
            ...
            if config.metadata then
                ...
                if item.name == 'repairparts' then
                    metadata = { durability = item.durability, upgrade = item.name, label = item.label, description = item.label .. ' Restore Parts Durability ', image = item.name }
                end
...
lib.callback.register('renzu_tuners:RepairPart', function(src, percent, noMetadata)
    ...
    if items then
        for k, v in pairs(items) do
            ...
            if v.metadata and v.metadata.durability and v.metadata.durability >= percent then
