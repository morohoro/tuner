GetWheelHandling = function(vehicle)
	...
	for i = 1 , GetVehicleNumberOfWheels(vehicle) do
		handling[i] = {label = label[i], health = ent.tires and ent.tires.tirehealth and ent.tires.tirehealth[i] or 100.0}	end
	return handling
end
...
HandleTires = function(vehicle,plate,default,state)
	...
	for i = 1 , GetVehicleNumberOfWheels(vehicle) do
		if tirehealth and math.random(1,100) < 50 then
			tirehealth[i] = tirehealth[i] - (tires.degrade or 0.1)
		end
		if tirehealth then
	...
	if not tiresave then tiresave = 0 end
	tiresave = tiresave + 1
	if tiresave > 60 then
		tiresave = 0
		...
		local Drift = function(handling)
			for index, value in ipairs(handling) do
				...
				elseif value[1] == 'vecInertiaMultiplier' or value[1] == 'vecCentreOfMassOffset' and vehicle ~= 0 then
					SetVehicleHandlingVector(vehicle, "CHandlingData", value[1], vector3(tonumber(value[2]), tonumber(value[2]), tonumber(value[2])))
			...
			applyVehicleMods(vehicle ~= 0 and vehicle or GetVehiclePedIsIn(PlayerPedId(), false), 0.0)
		end
		if mode ~= 'DRIFT' then
