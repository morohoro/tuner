Dyno = function(data,index)
	...
	local dynostate = dynotaken and Entity(vehicle).state and Entity(vehicle).state.dyno	local dynodriver = GetPedInVehicleSeat(GetVehiclePedIsIn(cache.ped),-1) == cache.ped
...
SetVehicleManualGears = function(vehicle,dyno,auto,eco)
	...
	local ecu_gears = ecu[plate] and ecu[plate].active and ecu[plate].active.gear_ratio
	if ecu_gears then
		customgears = {}
 ...
 Citizen.CreateThreadNow(function() -- realtime update from upgrades
     while dyno do
         local data = ecu
         boostpergear = data[plate] and data[plate].active and data[plate].active.boostpergear or {}
         vehicle_gear_ratio[maxgear] = data[plate] and data[plate].active and data[plate].active.gear_ratio or gears[maxgear]
         upgrade, stats = GetEngineUpgrades(vehicle) -- get current upgraded parts
         tune = GetTuningData(plate) -- get current tuned data
         efficiency, Fuel_Air_Volume, maxafr = EngineEfficiency(vehicle,stats,tune,rawturbopower)
